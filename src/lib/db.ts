...
async function createIndexes(db: Db) {
  await Promise.all([
    ...
    db.collection("referral_clicks").createIndex({ referrerClerkId: 1, createdAt: -1 }),
    db.collection("referral_clicks").createIndex({ ipHash: 1, code: 1, createdAt: -1 }),
    db.collection("referral_clicks").createIndex({ code: 1, converted: 1, createdAt: -1 }),
    db.collection("referral_clicks").createIndex({ refereeClerkId: 1 }, { sparse: true }),
