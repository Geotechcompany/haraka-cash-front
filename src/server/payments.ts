...
export const listPaymentTransactions = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return payments.map((payment) =>
    toPayment(payment, payment.applicationNumber ? applicantByApp.get(payment.applicationNumber) : undefined),
  );
});
...
async function markApplicationDisbursing(applicationNumber: string) {
  ...
  const application = await db.collection<ApplicationRecord>("applications").findOne({ applicationNumber });
...
export async function handleSmplyPayWebhook(payload: unknown) {
  ...
  const status: PaymentStatus =
    parsed.status === "success" ? "success" : parsed.status === "failed" ? "failed" : payment.status;
  ...
  if (status === "success" && payment.kind === "processing_fee" && payment.applicationNumber) {
